...
function normalizeExtraction(extraction, sourceKind, sourceImage) {
    if (extraction && typeof extraction === 'object' && !Array.isArray(extraction)) {
        return {
            status: String(extraction.status || 'needs-review').trim().toLowerCase(),
...
export function normalizeCard(rawCard, gameName, sourceKind = 'legacy') {
    ...
    const extraction = normalizeExtraction(rawCard?.extraction, sourceKind, sourceImage);
    ...
    const normalized = {
        ...
        renderMode: sourceKind === 'rich' && ['auto', 'verified'].includes(extraction.status)
            ? 'rich'
            : 'image',
    ...
    normalized.searchText = buildCardSearchText(normalized);
...
export function mergeCardCatalogs(legacyCatalog = {}, richCatalog = null) {
    ...
    const manifestTypes = {
        sentryTypes: toArray(richManifest.sentryTypes || legacyCatalog.sentryTypes),
        corrupterTypes: toArray(richManifest.corrupterTypes || legacyCatalog.corrupterTypes),
        heldBackCardTypes: toArray(richManifest.heldBackCardTypes || legacyCatalog.heldBackCardTypes)
    };
...
export function normalizeCachedCardCatalog(cachedCatalog = {}) {
    const catalog = toRecord(cachedCatalog);
    const games = {};

    Object.entries(toRecord(catalog.games)).forEach(([gameName, cards]) => {
        games[gameName] = toArray(cards).map(card => {
            const extractionStatus = String(card?.extraction?.status || '').trim().toLowerCase();
            const sourceKind = card?.renderMode === 'rich' || ['auto', 'verified'].includes(extractionStatus)
                ? 'rich'
                : 'legacy';
            return normalizeCard(card, gameName, sourceKind);
        });
    });

    return {
        sentryTypes: toArray(catalog.sentryTypes),
        corrupterTypes: toArray(catalog.corrupterTypes),
        heldBackCardTypes: toArray(catalog.heldBackCardTypes),
        games,
        icons: toRecord(catalog.icons),
        cardManifest: toRecord(catalog.cardManifest)
    };
}
...
export function searchCards(cards = [], rawQuery = '') {
    ...
    return toArray(cards)
        ...
        .sort((left, right) => {
            const leftTitle = String(left.card || '').toLowerCase();
            const rightTitle = String(right.card || '').toLowerCase();
