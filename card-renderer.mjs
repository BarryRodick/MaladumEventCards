...
function createFooter(doc, card, options, { compact = false } = {}) {
    ...
    ['left', 'right'].forEach(side => {
        const items = (Array.isArray(card.footer[side]) ? card.footer[side] : [])
            .filter(item => item && (!compact || item.type === 'icon'));
        if (items.length === 0) {
            return;
        }
...
function renderImageCard(doc, card, size = 'full') {
    ...
    image.src = `cardimages/${card.sourceImage || card.contents || ''}`;
    image.alt = card.card || 'Card image';
    figure.appendChild(image);

    if (size !== 'compact') {
        const caption = doc.createElement('figcaption');
        caption.className = 'visually-hidden';
        caption.textContent = `${card.card || 'Card'} (${card.type || ''})`;
